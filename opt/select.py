...
class Select():
    ...
    def ucb(self, prompt_list):
        ...
        if not index_list:
            raise ValueError("[ERROR] index_list is empty in UCB. Upstream prompt_list was empty.")
    ...
    def run(self, prompt_list):
        print(f"[DEBUG] Select.run called with {len(prompt_list)} prompts")
        if not prompt_list:
            raise ValueError("[ERROR] No prompt candidates passed to Select.run!")
